...
// valLetters: checks whether in the control input_field there are only letters ( or apostrophes, commas, hyphen or space ). If not in the message_span the message is shown.
...
function valLetters(input_field, message_span, required) {
    var regex = /^[a-zA-Z\x27\x20\x2C\x2D]+$/;
    if (!input_field.value == '') {
        if ( !regex.test(input_field.value)) { // if the user entered some characters which are not letters ( in the input_field )
            message_span.innerHTML = "* Can contain only letters, apostrophes, commas, hyphen and space";
