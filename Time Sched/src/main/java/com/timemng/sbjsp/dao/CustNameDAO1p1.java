...
/*
// getName - retrieving name of the customer with the certain customer ID 
public List<CustNameInfo1p1> getName() {
	List<CustNameInfo1p1> list = null; // initialising the list
	
	// SQL_CUST_NAME - the String that contains the part of the query onto which the WHERE clause was added 
	String sql = CustNameMapper1p1.SQL_CUST_NAME;
	try {
	 	...
	 	// running the query and retrieving the list of customer names with the specified customer ID
	 	list = this.getJdbcTemplate().query(sql, params, mapper);
	 } catch (Exception e) {
	 	
	 }
	return list; // return the name for the customer with the certain customer ID
}
*/ 
...
/*
// add to the SQL query the WHERE clause - where ( cust_id = id of the customer ) 
public void addToQueryStrName(String custID ) {

	// if other customer used Contact Us page before then the original SQL_CUST_NAME got changed so I have to reset it to its original value 
	CustNameMapper1p1.resetSQL_CUST_NAME();
	String sql = CustNameMapper1p1.SQL_CUST_NAME;
	
	// I am changing the SQL query to return the customer's first and last name ( for the given customer's ID ) 
	sql += " where cust_id='" + custID + "' ";
