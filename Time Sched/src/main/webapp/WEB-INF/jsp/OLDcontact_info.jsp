...
<div class="w3-content">
    ...
    %>	
    		<form id="contact_form2" name="contact_form" action="http://localhost:8080/cust_message" method="post"> <!-- if the customer didn't already log in -->
    <%
        }
    ...
    %>
		...
		<div class="w3-section">
			...
			<!--  when removing REQUIRED go to MainController, show_schedule, and in method show_schedule remove required=true
			      for the last-name -->
			<input class="w3-input w3-border" type="text" name="email" maxlength="40"> <!-- input field for entering the email -->
		</div> 
		<div class="w3-section">
			<label>Message - FIX this in the DB</label> 
			<!--  ??????????????????????????????  -->
			<!--  when removing REQUIRED go to MainController, show_schedule, and in method show_schedule remove required=true
			      for the last-name -->
			<!-- <input class="w3-input w3-border" type="text" name="message" required=true> <!-- input field for entering the message -> -->
			<textarea class="w3-input w3-border" id="message" name="message" rows="3" maxlength="1" required=true></textarea>
		</div> 
